"use client";

import { useState } from "react";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { CalendarCheck, CheckCircle } from "lucide-react";
import Input from "@/components/ui/Input";
import Textarea from "@/components/ui/Textarea";
import Button from "@/components/ui/Button";
import { visitRequestSchema, type VisitRequestInput } from "@/lib/validations";

interface VisitRequestFormProps {
  propertyId: string;
  propertyTitle: string;
}

export default function VisitRequestForm({
  propertyId,
  propertyTitle,
}: VisitRequestFormProps) {
  const [submitted, setSubmitted] = useState(false);

  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<VisitRequestInput>({
    resolver: zodResolver(visitRequestSchema),
  });

  const onSubmit = async (data: VisitRequestInput) => {
    const res = await fetch("/api/visits", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...data, property_id: propertyId }),
    });
    if (res.ok) setSubmitted(true);
  };

  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-8 text-center">
        <CheckCircle className="w-12 h-12 text-green-500 mb-3" />
        <h3 className="text-lg font-semibold text-[#0f1724] mb-1">
          Demande envoyée !
        </h3>
        <p className="text-sm text-gray-500">
          Nous vous contacterons très prochainement pour confirmer votre visite.
        </p>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-6">
      <div className="flex items-center gap-3 mb-5">
        <div className="w-10 h-10 bg-[#1a3a5c]/10 rounded-xl flex items-center justify-center">
          <CalendarCheck className="w-5 h-5 text-[#1a3a5c]" />
        </div>
        <div>
          <h3 className="font-semibold text-[#0f1724]">Demander une visite</h3>
          <p className="text-xs text-gray-500 line-clamp-1">{propertyTitle}</p>
        </div>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <Input
          label="Nom complet"
          placeholder="Votre nom"
          error={errors.full_name?.message}
          {...register("full_name")}
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Input
            label="Email"
            type="email"
            placeholder="votre@email.com"
            error={errors.email?.message}
            {...register("email")}
          />
          <Input
            label="Téléphone"
            type="tel"
            placeholder="+221 00 000 00 00"
            error={errors.phone?.message}
            {...register("phone")}
          />
        </div>
        <Input
          label="Date souhaitée"
          type="date"
          className="h-11"
          error={errors.preferred_date?.message}
          {...register("preferred_date")}
        />
        <Textarea
          label="Message (optionnel)"
          placeholder="Précisez vos disponibilités ou questions..."
          rows={3}
          {...register("message")}
        />
        <Button type="submit" loading={isSubmitting} className="w-full" size="lg">
          Envoyer la demande
        </Button>
      </form>
    </div>
  );
}
