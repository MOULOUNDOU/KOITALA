  ...
  return (
    <Link href={`/biens/${property.slug}`} className={cn("block group", className)}>
      <article className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
        ...
        <div className="p-3">
          {/* Type */}
          <span className="inline-block px-2 py-0.5 rounded-md bg-[#f4f6f9] text-[11px] font-semibold text-[#1a3a5c] mb-1.5">
          ...
          <div className="flex items-center gap-1 mb-2">
            ...
            <span className="text-[12px] text-gray-500 truncate">{property.city}</span>
          </div>
          {/* Price */}
          <p className="text-[15px] font-extrabold text-[#1a3a5c]">
            {formatPrice(property.price)}
            {property.listing_type === "location" && (
              <span className="text-[11px] text-gray-400 font-normal"> /mois</span>
            )}
          </p>
        </div>
      </article>
